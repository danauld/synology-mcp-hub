...
# whatsupp-mcp (Node Express HTTP MCP) exposes /health (returns JSON).
curl -fsS --max-time 5 "http://127.0.0.1:8129/health" >/dev/null
...
curl -fsS --max-time 5 -X POST "http://127.0.0.1:8127/mcp" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json, text/event-stream" \
    -d '{"jsonrpc":"2.0","id":"hc","method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"healthcheck","version":"1.0"}}}' \
    >/dev/null
